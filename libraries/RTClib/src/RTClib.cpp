...
/**************************************************************************/
/*!
	@brief  Start nRF52 RTC
*/
/**************************************************************************/

boolean RTC_nRF52::begin(void) {
	nrf_rtc_int_enable(NRF_RTCZ, NRF_RTC_INT_OVERFLOW_MASK);
	nrf_rtc_event_enable(NRF_RTCZ, NRF_RTC_EVENT_OVERFLOW);
	nrf_rtc_prescaler_set(NRF_RTCZ, RTC_FREQ_TO_PRESCALER(8)); // 8 Hz
...
DateTime RTC_nRF52::now(void) {
	...
	return lastUnix + elapsedSeconds;
}

RTC_nRFX *rtcPointer = NULL;

static void rtc_handler(nrfx_rtc_int_type_t int_type) {
	rtcPointer->handleInterrupt(int_type);
}
...
boolean RTC_nRFX::begin(void) {
		
	rtcPointer = this;
		
	nrfx_rtc_config_t config = NRFX_RTC_DEFAULT_CONFIG;
	config.prescaler = RTC_FREQ_TO_PRESCALER(8); // 8 Hz
...
*/
/**************************************************************************/
void RTC_nRFX::adjust(const DateTime &dt) {
	uint32_t elapsedSeconds = nrfx_rtc_counter_get(&rtc) >> 0x3;
	lastUnix = dt.unixtime() - elapsedSeconds;
}
...
*/
/**************************************************************************/
DateTime RTC_nRFX::now(void) {
	uint32_t elapsedSeconds = nrfx_rtc_counter_get(&rtc) >> 0x3;
	return lastUnix + elapsedSeconds;
}

void RTC_nRFX::handleInterrupt(const nrfx_rtc_int_type_t int_type) {
